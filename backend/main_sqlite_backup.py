...
# Copy database to /tmp on first run
SOURCE_DB = os.path.join(os.path.dirname(__file__), 'students.db')
DB_FILE = '/tmp/students.db'

# Copy database to /tmp if not exists or source is newer
if not os.path.exists(DB_FILE) or os.path.getmtime(SOURCE_DB) > os.path.getmtime(DB_FILE):
    shutil.copy2(SOURCE_DB, DB_FILE)
    print(f"Database copied to {DB_FILE}")
